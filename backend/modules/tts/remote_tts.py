"""
JARVIS — Remote TTS backend.

Calls an external TTS HTTP server (e.g. xtts-api-server) and returns WAV bytes.
Decouples heavy TTS ML dependencies from the main JARVIS process.
...
logger = get_logger("jarvis.tts.remote")
...
class RemoteTTS:
    ...
    async def check_health(self) -> bool:
        ...
        """
        if not _HTTPX_OK:
            logger.warning("[JARVIS] RemoteTTS: httpx not installed")
            ...
            for ep in endpoints:
                try:
                    ...
                    if r.status_code < 500:
                        ...
                        logger.info(f"[JARVIS] RemoteTTS: server online at {self._host} (probe {ep} -> {r.status_code})")
                        return True
                except Exception:
                    continue

        self._available = False
        logger.warning(f"[JARVIS] RemoteTTS: server not reachable at {self._host}")
        return False
    ...
    async def synthesise(self, text: str) -> bytes:
        ...
        """
        if not _HTTPX_OK:
            raise TTSError("[JARVIS] RemoteTTS: httpx not installed")
        if not text.strip():
            return b""
        ...
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            ...
            try:
                ...
                if r.status_code == 200 and r.content:
                    logger.info(f"[JARVIS] RemoteTTS: xtts-api-server -> {len(r.content)} bytes")
                    self._available = True
                    return r.content
                if r.status_code not in (404, 405):
                    raise TTSError(f"[JARVIS] RemoteTTS: server returned {r.status_code}: {r.text[:200]}")
            except TTSError:
                raise
            except Exception as exc:
                logger.debug(f"[JARVIS] RemoteTTS: xtts-api-server endpoint failed ({exc}), trying OpenAI format")
            ...
            try:
                ...
                if r.status_code == 200 and r.content:
                    logger.info(f"[JARVIS] RemoteTTS: OpenAI-compat -> {len(r.content)} bytes")
                    self._available = True
                    return r.content
                raise TTSError(f"[JARVIS] RemoteTTS: OpenAI endpoint returned {r.status_code}: {r.text[:200]}")
            except TTSError:
                raise
            except Exception as exc:
                self._available = False
                raise TTSError(f"[JARVIS] RemoteTTS: all endpoints failed: {exc}") from exc
    ...
    def set_speaker(self, speaker: str) -> None:
        ...
        self._speaker = speaker
        logger.info(f"[JARVIS] RemoteTTS: speaker -> '{speaker}'")

    def set_language(self, language: str) -> None:
        """Update the synthesis language at runtime."""
        self._language = language
        logger.info(f"[JARVIS] RemoteTTS: language -> '{language}'")
